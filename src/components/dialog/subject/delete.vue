<template>
  <v-dialog v-model="dialog" width="500" v-if="jwt">
    ...
    <v-card dark class="dialog">
      <v-container>
        <v-card-title class="headline">Supprimer le sujet {{name}}</v-card-title>
        <v-card-text>
          <v-text-field
                v-model="title"
                :rules="titleRules"
...
export default {
  ...
  methods: {
    ...
    postDelete () {
      axios({
        method: 'post',
        url: `https://api.sepo.tk/public/api/subject/${this.name}/delete`,
        headers: {
        ...
        .then(response => {
          ...
          let body = response.data.error ? 'Wrong title' : `Subject<deleted>${this.title}</deleted>`
